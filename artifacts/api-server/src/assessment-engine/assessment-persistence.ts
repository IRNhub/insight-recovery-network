...
export function storedAssessmentFromRow(row: Assessment): StoredAssessmentResult | null {
  ...
  const screening = row.screeningClassification as ScreeningClassification;
  ...
  async create(record, accessTokenHash) {
    try {
      const created = await db.transaction(async (tx) => {
        ...
        await tx.insert(assessmentDeliveriesTable).values([
          { assessmentId: row.id, channel: "email" },
          { assessmentId: row.id, channel: "irn_os" },
        ]);
        return row;
      });
